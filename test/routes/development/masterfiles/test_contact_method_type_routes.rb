...
class TestContactMethodTypeRoutes < RouteTester
  ...
  def test_update
    ...
    ensure_exists!(INTERACTOR)
    row_vals = Hash.new(1)
    INTERACTOR.any_instance.stubs(:update_contact_method_type).returns(ok_response(instance: row_vals))
  ...
  def test_update_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:update_contact_method_type).returns(bad_response)
    Development::Masterfiles::ContactMethodType::Edit.stub(:call, bland_page) do
      patch_as_fetch 'development/masterfiles/contact_method_types/1', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
  ...
  def test_delete
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:delete_contact_method_type).returns(ok_response)
  ...
  def test_create
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_contact_method_type).returns(ok_response)
  ...
  def test_create_fail
    authorise_pass!
    ensure_exists!(INTERACTOR)
    INTERACTOR.any_instance.stubs(:create_contact_method_type).returns(bad_response)
    Development::Masterfiles::ContactMethodType::New.stub(:call, bland_page) do
      post_as_fetch 'development/masterfiles/contact_method_types', {}, 'rack.session' => { user_id: 1, last_grid_url: DEFAULT_LAST_GRID_URL }
    end
    expect_bad_page
